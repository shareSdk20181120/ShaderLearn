...
//对比高光逐顶点、逐像素光照来说，Blin-Phong光照模型的高光反射部分看起来更大，更亮一些。在实际渲染中我们会选择使用Blinn-Phong光照模型。这两种光照模型都是经验模型，Blinn-Phong模型更符合实验结果
Shader "ShaderLearn/BlinnPhongShader"
{
	Properties
	{
		...
		SubShader
	{
		...
		//前向渲染
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 worldLight = normalize(_WorldSpaceLightPos0.xyz);//这种方式只适用于平行光  只有是前向渲染时，_WorldSpaceLightPos0才被正确赋值
				fixed3 diffuse = _LightColor0.rgb * _Diffuse.rgb * saturate(dot(worldNormal,worldLight));
				//计算高光反射	这个计算 方式 ，没有使用反射方向
				fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - i.worldPos);//这种方式只适用于平行光 观察方向--世界空间下，摄像机坐标位置减去顶点坐标  
