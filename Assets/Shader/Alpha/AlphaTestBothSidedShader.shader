/*
透明度测试的双面渲染，之前我们写的透明度效果shader，都无法看到物体的内部结构，这是因为引擎踢除了背面的渲染图元，只渲染了正面的。
为得到正确的效果，使用Cull Off命令，关闭剔除功能.这时候需要渲染的图元数目会成倍的增加，除非用于特殊效果，通常情况下是不会关闭剔除功能的
*/
Shader "ShaderLearn/AlphaTestBothSidedShader"
{
	Properties
	{
